...
\newthought{Much has been said} about the so-called Standard Model (SM) of particle physics, so only the bare essentials of electroweak symmetry breaking and Higgs production relevant to SM \vhbb\,will be addressed here, following the discussion and notation of \cite{pdg} Chapter 11.  We then move onto the treatment of kinematic variables in collider events, including the two novel schemes considered in this thesis, the Lorentz Invariants (LI) and RestFrames (RF) variables.
...
\emph{Maybe do the 0-lep calculation and some cute RF diagrams}%Insert discussion of recursive jigsaw reconstruction?  Maybe replicate}
