...
The remaining signal systematics are analysis channel specific and are summarized in Table \ref{tab:vhSysSummaryAna}.  The methodologies match those described in Section \ref{sec:modsysgen}.  The UE+PS systematics were derived using the alternate samples mentioned above; QCD scale uncertainties were derived by varying scales by 1/3 and 3; and PDF uncertainties were derived by comparing the nominal set with the PDF4\-LHC\-15\-\_30 PDF set \cite{pdf4lhc}.
...
%		  & \multicolumn{2}{c ||}{0L: $ZH\to\nu\nu b\bar{b}$}	& \multicolumn{2}{c ||}{1L: $WH\to\ell\nu b\bar{b}$} 	& \multicolumn{2}{c }{2L: \ZH}  \\
		  & \multicolumn{2}{c ||}{0L}	& \multicolumn{2}{c ||}{1L} 	& \multicolumn{2}{c }{2L}  \\
...
\begin{table}[!htpb] \captionsetup{justification=centering}
\caption{ \label{tab:zjetsnorm} Effect of modeling systematics on $Z$+jets normalization in the 2lepton regions. For systematic uncertainties implemented with a prior the effect of 1-$\sigma$ variation is reported. 
\begin{center} 
\small 
...
\hline 
\end{tabular} 
The uncertainties labelled as $Zbb$ act on the entire $Z$+hf background.  Region labels ``lo'' and ``hi'' refer to event \ptv.} 
\end{center} 
\end{table} 
...
Single top sample are generated separately for the different production channels ($s$, $t$, and $Wt$) using \powheg\,with the CT10 NLO PDF's interfaced with \pythia6 using the PERUGIA2012 PS tune and the corresponding CTEQ6l1 LO PDF's and PHOTOS (TAUOLA) for QED final state ($\tau$) decays.%\footnote{No references were given in the note, and this background really isn't that important.}  
